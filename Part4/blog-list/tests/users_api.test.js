...
describe('users api with initial user', () => {
  beforeEach(async () => {
    ...
    const user = new User({ username: 'root', name: 'Admin Jones', passwordHash })
  ...
  describe('creating users', () => {
    ...
    test('creation fails with a missing username', async () => {
      ...
      const res = await api
        ...
        .send(newUser)
        .expect(400)
      assert.strictEqual(res.body.error, 'User validation failed: username: Path `username` is required.')
    ...
    test('creation fails with a too short username', async () => {
      ...
      const res = await api
        ...
        .send(newUser)
        .expect(400)
      assert.strictEqual(res.body.error, 'User validation failed: username: Path `username` (`hi`, length 2) is shorter than the minimum allowed length (3).')
